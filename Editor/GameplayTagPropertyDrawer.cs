...
namespace GameplayTag
{
    ...
    public class GameplayTagPropertyDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var buttonText = string.IsNullOrEmpty(tagNameProp.stringValue) ? "None" : tagNameProp.stringValue;
            if (GUI.Button(position, buttonText, EditorStyles.popup)) ShowTagSelector(position, tagNameProp);
        ...
        private void ShowTagSelector(Rect position, SerializedProperty property)
        {
            ...
            if (!GameplayTagManager.Instance.IsInitialized) GameplayTagManager.Instance.Initialize();

            // Get all tags and add to menu
            var allTags = GameplayTagManager.Instance.GetAllTags();
            foreach (var tag in allTags)
            {
                var fullName = tag.GetFullTagName();
